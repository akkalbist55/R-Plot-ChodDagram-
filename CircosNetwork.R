...
m <- data.frame(order = 1:6,
                Location = c("Bardia", "Chitwan", "Bajhang", "Kathmandu", "Pokhara", "Dang"),
...
data <- read.csv("~/RProject/data.csv")

m <- as.matrix(m[,c(1:6)])
dimnames(m) <- list(orig = df1$Location, dest = df1$Location)
#Sort order of data.frame and matrix for plotting in circos
df1 <- arrange(df1, order)
df1$Location <- factor(df1$Location, levels = df1$Location)
m <- m[levels(df1$Location),levels(df1$Location)]
...
### Sector details
circos.initialize(factors = df1$Location, xlim = cbind(df1$xmin, df1$xmax))

### Plot sectors
circos.trackPlotRegion(ylim = c(0, 1), factors = df1$Location, track.height=0.1,
...
df1 <- subset(df1, link.lwd = 2, link.lty = 2, link.border = "red")
df2 <- subset(df2, link.lwd = 2, link.lty = 2, link.border = "red")
...
for(k in 1:nrow(df2)){
  ...
  i<-match(df2$orig[k],df1$Location)
  j<-match(df2$dest[k],df1$Location)
  
  #plot link
  circos.link(sector.index1=df1$Location[i], point1=c(df1$sum1[i], df1$sum1[i] + abs(m[i, j])),
              sector.index2=df1$Location[j], point2=c(df1$sum2[j], df1$sum2[j] + abs(m[i, j])),
              col = df1$lcol[i])
